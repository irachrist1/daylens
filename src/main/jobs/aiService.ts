...
// Minimal block-level markdown → HTML: headings, tables, bullet/numbered lists,
// and paragraphs. Enough to render the grounded reports we generate cleanly.
function reportMarkdownToHtml(markdown: string): string {
  const blocks = markdown.split(/\n{2,}/).map((b) => b.trim()).filter(Boolean)
  const html: string[] = []
  for (const block of blocks) {
    const lines = block.split('\n').map((l) => l.trim()).filter(Boolean)
    const heading = lines[0].match(/^(#{1,4})\s+(.*)$/)
    const isTable = lines.length >= 2 && lines.some((l) => /^\|?[\s|:]*-{2,}[\s|:-]*$/.test(l) && l.includes('|'))
    if (isTable) {
      ...
      const headers = sepIdx >= 1 ? parseRow(lines[sepIdx - 1]) : []
      ...
      html.push('<table><thead><tr>' + headers.map((h) => `<th>${inlineMarkdownToHtml(h)}</th>`).join('') + '</tr></thead><tbody>'
        ...
        + '</tbody></table>')
    } else if (heading && lines.length === 1) {
      const level = heading[1].length
      html.push(`<h${level}>${inlineMarkdownToHtml(heading[2])}</h${level}>`)
    } else if (lines.every((l) => /^[-*]\s/.test(l))) {
      html.push('<ul>' + lines.map((l) => `<li>${inlineMarkdownToHtml(l.replace(/^[-*]\s+/, ''))}</li>`).join('') + '</ul>')
    } else if (lines.every((l) => /^\d+\.\s/.test(l))) {
      html.push('<ol>' + lines.map((l) => `<li>${inlineMarkdownToHtml(l.replace(/^\d+\.\s+/, ''))}</li>`).join('') + '</ol>')
    } else {
      html.push('<p>' + lines.map((l) => inlineMarkdownToHtml(l)).join('<br/>') + '</p>')
    }
  }
  return html.join('\n')
}

function buildReportHtml(title: string, subtitle: string, markdown: string): string {
  ...
  const showSubtitle = subtitle.trim().length > 0 && subtitle.trim() !== title.trim()
  return `<!doctype html><html><head><meta charset="utf-8"/><style>
    * { box-sizing: border-box; }
    ...
    ${showSubtitle ? `<p class="subtitle">${escapeHtml(subtitle)}</p>` : ''}
    ${reportMarkdownToHtml(markdown)}
    <footer>Generated by Daylens${showSubtitle ? ` for ${escapeHtml(subtitle)}` : ''}.</footer>
  </body></html>`
}

async function renderReportPdf(title: string, subtitle: string, markdown: string): Promise<Buffer> {
...
function renderReportDocx(title: string, subtitle: string, markdown: string): string {
  ...
  <style>
    ...
    <h1>${escapeHtml(title)}</h1>
    ${subtitle.trim() && subtitle.trim() !== title.trim() ? `<p class="subtitle">${escapeHtml(subtitle)}</p>` : ''}
    ${reportMarkdownToHtml(markdown)}
  </body></html>`
}
