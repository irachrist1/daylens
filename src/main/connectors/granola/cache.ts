 ...
 *  or markdown string, or Granola's ProseMirror document (text leaves only —
 *  attachments, marks, and unknown nodes contribute nothing). */
export function extractNoteLines(doc: Record<string, unknown>): string[] {
  const plain = typeof doc.notes_plain === 'string' && doc.notes_plain.trim()
    ...
    : typeof doc.notes_markdown === 'string' && doc.notes_markdown.trim()
      ? doc.notes_markdown
      : null
  const text = plain ?? proseMirrorText(doc.notes)
  if (!text) return []
  const lines: string[] = []
...
function calendarRange(doc: Record<string, unknown>): { startMs: number | null; endMs: number | null } {
  const event = doc.google_calendar_event
  if (!event || typeof event !== 'object') return { startMs: null, endMs: null }
  const record = event as Record<string, unknown>
  const timeOf = (value: unknown): number | null => {
    if (!value || typeof value !== 'object') return null
    return parseTime((value as Record<string, unknown>).dateTime)
  }
  return { startMs: timeOf(record.start), endMs: timeOf(record.end) }
}
...
export function parseGranolaCache(raw: string): GranolaCacheContent {
  ...
  for (const doc of documentsOf(state)) {
    ...
    const { startMs, endMs } = calendarRange(doc)
    docs.push({
      id,
