...
interface AIJobDefinition {
  ...
  // Suggestions are intentionally generated by the cheapest model for the
  // selected provider. This is visible as its own billed feature in Usage.
  forceEconomyModel?: boolean
...
function economyModelForProvider(provider: AIProviderMode): string {
  switch (provider) {
    ...
    case 'anthropic':
    default:
      return ANTHROPIC_TIER_MODELS.economy
  }
}
  ...
  for (const provider of orderedProviders) {
    ...
    configs.push({
      ...
      model: definition.forceEconomyModel
        ? economyModelForProvider(provider)
