...
export function buildPeriodPrompts(facts: WrappedPeriodFacts): { systemPrompt: string; userMessage: string } {
  ...
  const systemPrompt = [
    ...
    'Voice: warm but grounded, second-person, specific. Like Spotify Wrapped — personal, occasionally surprising. No motivational filler, no exclamation marks, no hedging ("likely", "approximately").',
