# Daylens feature registry

Living map: **Should** (from `docs/daylens-PMF.md`) vs **Now** (current app).

> **Round 3 folded in (2026-06-17).** Corrections and new rows from the council's
> winning plans are now merged here. Changes from Rounds 1–2 are marked
> **[CORRECTED]** (a registry "Now"/Status was revised) or **[NEW]** (a row the
> seed registry missed). The canonical assembled plan is
> [`DAYLENS-V2-PLAN.md`](DAYLENS-V2-PLAN.md). Truth rule still holds: code explains
> *why* something fails; only screenshots or live-app use can label something
> **works**.

**v2** = PMF vision reached by fixing the existing app — not greenfield, not fix-for-fix's-sake.

Status: **works** · **broken** · **untrusted** · **partial** · **missing** · **UNVERIFIED** (needs live test)

Evidence: `docs/plans/screenshots/`, app testing, founder.
...
**Founder-reported (not all captured in screenshots):** switch chat mid-generation → empty chat, sidebar chats gone, input disabled; navigate Apps → AI → chats disappear; new chat shows history until tab switch; all AI responses sound wrong; memory saves wrong patterns.
...
|---------|-------------|-----|--------|----------|
| Activity → time blocks | Sensible boundaries — one work stretch = one block; clear breaks | Arbitrary splits; 50 blocks/day; huge untracked gaps (8h+); same session fragmented | broken | timeline-today-*, founder |
...
| Feature | Should (v2) | Now | Status | Evidence |
|---------|-------------|-----|--------|----------|
| Day timeline | Scrollable reality calendar with proof; one day-payload feeds all sub-views | Renders but labels/categories/durations untrusted; each view recomputes | broken | timeline-day-jun16-reanalyze-shape-of-day.png, founder |
...
| Re-analyze with AI **[CORRECTED]** | Uses **Settings model**; refreshes synthesis; recovers on failure | Founder + codex live: uses **Gemini** (quota error) while Settings = Claude Haiku; stuck "Re-analyzing…" (verify) | broken | timeline-day-jun16-*, settings-ai-claude-haiku-connected.png, live |
...
| Main mode stat | Reflects actual **work** mode (separate leisure readout) | "Main mode: Entertainment" for founder/dev | untrusted | timeline-week-jun1-7-*, timeline-week-jun15-21-* |
...
| Feature | Should (v2) | Now | Status | Evidence |
|---------|-------------|-----|--------|----------|
| Period toggles (Today/Day/7d/30d) | Same app correct + same title scheme across all periods | 7d/30d populate; daily often empty/wrong; codex live: Today names better (verify) | broken | founder, apps-*, live |
...
| Chat sidebar persistence | History survives tab switches + navigation | **Gone after Apps → AI** (codex live, verify); empty mid-generation | broken | ai-new-chat-empty-sidebar.png, ai-chat-sidebar-with-history.png, founder |
...
| Switch chat during generation | Safe switch / cancel / background | Breaks: empty chat, no sidebar, input disabled | broken | founder |
| Input during generation | Usable or clearly blocked + recoverable | Disabled / broken during generation | broken | founder |
...
| Feature | Should (v2) | Now | Status | Evidence |
|---------|-------------|-----|--------|----------|
| AI provider + model | One provider; every surface honors it; errors name the selected provider | Claude Haiku set; not used by re-analyze (Gemini quota) | broken | settings-ai-claude-haiku-connected.png, founder |
...
| Work memory / patterns **[CORRECTED]** | Patterns improve naming/attribution with varied, earned confidence + shown evidence/impact | **All 19 patterns tagged "browsing" @ identical 65%** — incl. Teams, Claude, malaria report, Apple Dev Docs | broken | settings-work-memory-learned-patterns.png, founder |
...
| Locked / protected user edits **[NEW]** | Manual edits authoritative until explicit reset; re-analysis/rebuild never overwrite | AI re-analysis can overwrite manual edits | broken | founder, timeline-today-merge-down-fix-episode-panel.png |
| Trust bar | Stake a client answer on timeline/AI | Cannot | broken | all sections |

---

## Open work — cross-cutting gaps (council-flagged, not surface-specific)
...
This registry has been folded back in for Round 3. For the implementing agent:

1. Read [`DAYLENS-V2-PLAN.md`](DAYLENS-V2-PLAN.md) — the canonical, assembled plan.
2. Use these rows as the feature checklist; each **Now** is screenshot/founder
   evidence or marked `UNVERIFIED`.
3. **Re-screenshot before trusting any "Now"** — screenshots are dated mid-June
   2026 on app v1.0.44. Confirm `UNVERIFIED` rows by driving the app in Phases 0–1.
4. Green `npm test` / `timeline:eval` is **not** product truth — the dogfood day is.
