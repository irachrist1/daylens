...
- macOS: packaged auto-update proof is still blocked. In this pass the packaged app immediately failed its update check because `electron-updater` resolved the latest published GitHub release tag `v1.0.26-win`, which does not contain `latest-mac.yml`; until the mac release metadata/assets are published under the live release tag and re-tested, packaged updater behavior remains implemented pending verification.
