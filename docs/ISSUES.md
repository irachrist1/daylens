...
Ship-readiness closure pass on 2026-04-18 added persistent AI chat threads + a durable artifact library (`ai_threads` / `ai_artifacts` tables + on-disk files under `userData/artifacts/`) inside the AI surface, a Focus score V2 heuristic (coherence + deep-work density + artifact progress + demoted context-switching penalty) in `src/main/lib/focusScore.ts` and wired through `snapshotExporter.ts`, explicit Anthropic / OpenAI model selectors in the advanced AI settings block, simplified Timeline block-label override UI (one input + Save / Reset), independently scrolling Timeline inspector columns, broad Settings copy cleanup, a macOS auto-update support branch in `updater.ts`, and new PostHog events (`focus_session_started` / `focus_session_stopped` / `artifact_created` / `ai_thread_created` / `ai_thread_archived` / `ai_thread_deleted`) that flow through the existing PII-safe allow-list scrubber. All typechecking, the full `npm run test:ai-chat` suite (30/30), the new `tests/focusScoreV2.test.ts` (5/5), and `npm run build:all` pass in this repo; real end-to-end provider runs, packaged-install auto-update proof, and live PostHog ingestion still remain implemented pending verification.
