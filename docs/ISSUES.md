...
- macOS: packaged auto-update proof is still blocked in the currently published release. In this pass the packaged app immediately failed its update check because `electron-updater` resolved the latest published GitHub release tag `v1.0.26-win`, which does not contain `latest-mac.yml`. The branch now updates the release workflows so macOS, Windows, and Linux publish their updater metadata to the shared version release tag (`vX.Y.Z`) instead of split platform releases, but that fix still needs a real release run and re-test before updater behavior should be called verified.
