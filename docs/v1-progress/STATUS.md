...
Last updated: 2026-05-28 after shipping the F0-F7 V1 closeout fixes on `v1/main`. Apps Generate v3 remains code-proven: `generateAppNarrative` no longer short-circuits before the model when `force=true`, errors propagate to the renderer banner, and structured `[ai] app_narrative` / `[apps-narrative]` logs make any remaining failure observable in dev. Resume review also fixed the Week Review explicit Generate/Refresh path so `force=true` bypasses the signature cache there too; before this, the new button could still silently reuse a cached week review when evidence was unchanged. Typecheck and the focused Apps top-domains, recap, and block-label tests pass locally.

## V1 closeout ledger

| ID | Shipped on `v1/main` | Commit(s) | Proof |
|---|---|---|---|
| F0 | Windows and Linux installers are live on the stable `v1.0.36` GitHub release used by daylens.io downloads. | release assets from `v1.0.36-preview.1` / run `26581779077`; upload claim `ab0d1f6` | [v1.0.36 release](https://github.com/irachrist1/daylens-v1/releases/tag/v1.0.36) lists `.exe`, `.AppImage`, `.deb`, `.rpm`, `latest.yml`, and `latest-linux.yml` |
...
| F2 | Video/social/streaming page artifacts route blocks out of `development`, and focus scoring is loosened for steady deep-work days. | `219fd99`, `57c65d5` | `tests/blockLabelerCategoryFit.test.ts`, `tests/focusScoreV2.test.ts`, `npm run typecheck` |
...
## Prompt closeout ledger

| Item | Shipped on `v1/main` | Commit(s) | Proof |
|---|---|---|---|
| A | Browser page artifacts no longer own IDE/terminal-dominant development block labels. | `aca7e11`, `288a612` | `tests/blockOwnership.test.ts` covers Kiro/Ghostty plus concurrent Safari YouTube. |
| B | `getRecapRange` lightweight payloads hydrate real sessions, websites, segments, `appCount`, and `siteCount` for recap consumers. | `6c577e5`, test seed in `ded84b8` | `tests/recapRange.test.ts` asserts every recap-consumed field is populated. |
| C | YouTube/video/social page artifacts categorize as entertainment/social, and steady dev days with modest drift score in the 75-85 focus band. | `219fd99`, `57c65d5` | `tests/blockLabelerCategoryFit.test.ts` and `tests/focusScoreV2.test.ts`. |
...
- The persisted-block fast path now has recap parity coverage, but live UI screenshot validation is still separate from unit proof.
...
| 3 | AI/overall performance | recap payload parity fixed; still needs measured runtime timings | `code-proven`/`tested`: `getRecapRange` now hydrates real sessions, websites, segments, `appCount`, and `siteCount` in the lightweight path (`6c577e5`), with `tests/recapRange.test.ts` asserting recap-consumed fields. Week review correctness is addressed at the data layer (cache-only read + scopeKey verification). | Validate Week review fix on Desktop and add measured timings. |
...
Code review of the `75f003e` v1-polish commit was completed on 2026-05-28 during the resume batch. One P1-style issue was fixed: Week Review's explicit Generate/Refresh path accepted `force=true` at IPC/export level but still let `generateWeekReview` return the cached row on signature match. That path now honors force end-to-end. The lightweight recap payload parity concern is now closed by `6c577e5`.
...
3. ~~**`getRecapRange` returns incomplete payloads.**~~ Fixed on 2026-05-28 (`6c577e5`): the lightweight path hydrates real day sessions, websites, timeline segments, `appCount`, and `siteCount`, and block member sessions/websites/key pages are populated from stored evidence plus DB reads. `tests/recapRange.test.ts` covers the consumer fields.
...
| Focused unit tests | passed locally on 2026-05-28: `tests/appsTopDomains.test.ts`, `tests/recap.test.ts`, `tests/blockLabel.test.ts`, `tests/workMemory.test.ts`, `tests/blockOwnership.test.ts`, `tests/recapRange.test.ts`, `tests/blockLabelerCategoryFit.test.ts`, `tests/focusScoreV2.test.ts` (40/40) |
...
- ~~**Focus score feels low.**~~ Addressed as part of the entertainment-categorization fix above (`219fd99`) plus the steady deep-work continuity credit (`57c65d5`) — video-artifact blocks no longer count as focus-eligible development time, and repeated 25m+ focus blocks can land a real work day in the 75-85 band instead of pinning it near 66.
