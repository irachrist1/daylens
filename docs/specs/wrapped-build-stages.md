...
| 1 — Content + benchmark | **Substantially converged** 2026-07-08. Full slide catalog (`docs/wrapped-slide-catalog.md`), a real-AI/real-DB benchmark (`tests/wrappedBenchmark.test.ts` + `tests/wrapped-bench/`, `npm run wrapped:bench`), and the recursive improvement loop (history in `docs/wrapped-benchmark-log.md`). Content went from an all-fallback ~6.4 baseline to day-deck averages ~8.9 typical / 9.0-9.3 on good runs, essentially every slide an AI line scoring 8-10; week deck ~8.0-8.3. Root causes fixed: wrap-job timeouts (22s→40s) that collapsed whole decks; the showcase riding the Haiku/balanced tier (→quality); guard caps and clock/word rules killing good lines; a keyhole judge (now scores accuracy against whole-day facts). **Not a deterministic pass:** run-to-run variance (~±0.4) plus the `focus` slide's structural motion ceiling and corrupt idle-detection data on some fixtures mean a single run does not always clear the hard avg≥9 bar even though content is consistently strong; honest per-slide non-convergence notes are in the benchmark log. Founder decision: rubric/catalog delegated to the agent to own; scoring measures the real AI line (a fallback counts as a failure), no static template anywhere. |
