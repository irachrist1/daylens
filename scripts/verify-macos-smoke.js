...
if (!reportPathArg || !windowStatePath) {
  fail('Usage: node scripts/verify-macos-smoke.js --report <path> --window-state <path>')
}

const reportPath = path.resolve(reportPathArg)
if (!fs.existsSync(reportPath)) fail(`Report file does not exist: ${reportPath}`)
const report = JSON.parse(fs.readFileSync(reportPath, 'utf8'))

if (report.ok !== true)
  fail(
    `App reported failure at stage "${report.stage ?? 'unknown'}": ${report.error ?? 'unknown error'}`,
  )
if (report.platform !== 'darwin') fail(`Expected darwin platform, got: ${report.platform}`)
if (report.isPackaged !== true) fail('Expected a packaged app runtime.')
...
console.log('macOS smoke verification passed:')
