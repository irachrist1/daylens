...
const cases = [
    ...
    { name: 'ASYV cumulative time', question: 'How many hours have I spent on ASYV today?', expectRouted: true,
        check: (a) => { strict_1.default.match(a, /2h 10m/i); strict_1.default.match(a, /outlook|excel|localhost|terminal|vs code/i); } },
    { name: 'ASYV title enumeration', question: 'Which ASYV titles matched today?', expectRouted: true,
        check: (a) => { strict_1.default.match(a, /ASYV kickoff notes/i); strict_1.default.match(a, /ASYV budget model/i); } },
    { name: 'ASYV app breakdown', question: 'Break ASYV down by app today.', expectRouted: true,
        check: (a) => { strict_1.default.match(a, /Visual Studio Code|VS Code/i); strict_1.default.match(a, /Outlook/i); strict_1.default.match(a, /Excel/i); } },
    ...
    { name: 'Biggest distraction — no duplicates', question: 'What distracted me today?', expectRouted: true,
        check: (a) => {
            strict_1.default.match(a, /reddit|chrome|r\/programming|entertainment/i);
            const label = a.match(/^(.+?) was/)?.[1] ?? '';
            if (label)
                strict_1.default.ok(a.indexOf(label, a.indexOf(label) + label.length) === -1, `"${label}" duplicated: ${a}`);
        } },
    // ── Entity identity ─────────────────────────────────────────────────────────
    { name: 'Who is ASYV', question: 'Who is ASYV?', expectRouted: true,
        check: (a) => { strict_1.default.match(a, /ASYV/i); strict_1.default.match(a, /client|project|develop/i); } },
    { name: 'What do I do for ASYV', question: 'What do I do for ASYV?', expectRouted: true,
        check: (a) => { strict_1.default.match(a, /ASYV/i); strict_1.default.match(a, /develop|coding|client/i); } },
    ...
    { name: 'Who is Acme Corp', question: 'Who is Acme Corp?', expectRouted: true,
        check: (a) => { strict_1.default.match(a, /Acme Corp/i); strict_1.default.match(a, /client|proposal|email|doc/i); } },
    // ── Client listing ──────────────────────────────────────────────────────────
    { name: 'List all my clients today', question: 'List all my clients today.', expectRouted: true,
        check: (a) => { strict_1.default.match(a, /ASYV/i); strict_1.default.match(a, /Acme/i); } },
    ...
    { name: 'Compare ASYV vs Acme Corp', question: 'Compare ASYV vs Acme Corp today.', expectRouted: true,
        check: (a) => { strict_1.default.match(a, /ASYV/i); strict_1.default.match(a, /Acme Corp/i); strict_1.default.match(a, /\d+h|\d+m/); } },
    ...
    { name: 'Summarize my day', question: 'Summarize my day.', expectRouted: true,
        check: (a) => { strict_1.default.ok(a.length > 100); strict_1.default.match(a, /tracked|focused|apps/i); } },
    ...
    { name: 'Focus score', question: 'What is my focus score today?', expectRouted: true,
        check: (a) => { strict_1.default.match(a, /focus score/i); strict_1.default.ok(!a.includes('client or project'), 'Must not misroute to entity identity'); } },
...
async function main() {
    ...
    for (const tc of cases) {
        const routed = await (0, insightsQueryRouter_1.routeInsightsQuestion)(tc.question, ANCHOR, context, db);
        if (routed)
            context = routed.resolvedContext;
        ...
        if (routed)
            console.log(`     ${routed.answer.replace(/\n/g, ' ').slice(0, 120)}`);
        else
