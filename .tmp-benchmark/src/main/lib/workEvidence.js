...
exports.deriveWorkEvidenceSummary = deriveWorkEvidenceSummary;
exports.formatWorkEvidenceSummary = formatWorkEvidenceSummary;
const types_1 = require("@shared/types");
const WEBSITE_CATEGORY_RULES = [
...
function inferWebsiteCategory(site) {
...
function collectAppSignals(appSummaries) {
...
function collectSessionSignals(sessions) {
...
function collectWebsiteSignals(websiteSummaries) {
...
function scoreSignal(signal) {
    const focusedBonus = types_1.FOCUSED_CATEGORIES.includes(signal.category) ? 1.1 : 1;
...
function taskLabelForCategory(category, signals) {
    const topLabels = signals.slice(0, 3).map((signal) => signal.label.toLowerCase());
    if (category === 'development')
        return topLabels.some((label) => label.includes('github') || label.includes('code')) ? 'Development work' : 'Coding work';
    if (category === 'aiTools')
        return 'AI-assisted work';
    if (category === 'communication')
        return 'Communication work';
    if (category === 'email')
        return 'Email work';
...
function summarizeSignals(signals, limit) {
    if (signals.length === 0)
...
function deriveWorkEvidenceSummary(input) {
    const signals = [
    ...
    const focusedSeconds = signals
        .filter((signal) => types_1.FOCUSED_CATEGORIES.includes(signal.category))
...
function formatWorkEvidenceSummary(summary) {
