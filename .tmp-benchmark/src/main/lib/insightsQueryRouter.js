...
exports.routeInsightsQuestion = routeInsightsQuestion;
const queries_1 = require("../db/queries");
const types_1 = require("@shared/types");
const focusScore_1 = require("../lib/focusScore");
const workEvidence_1 = require("../lib/workEvidence");
...
function isFocusedSession(session) {
    return session.isFocused || types_1.FOCUSED_CATEGORIES.includes(session.category);
}
function isDistractingSignal(signal) {
    return signal.category === 'entertainment' || signal.category === 'social';
}
function isNonFocusSignal(signal) {
    return !types_1.FOCUSED_CATEGORIES.includes(signal.category) && signal.category !== 'system';
}
function formatSignal(signal) {
    return `${signal.label} (${formatDuration(signal.seconds)})`;
}
function formatSignalList(signals, limit = 3) {
    return signals.slice(0, limit).map(formatSignal).join(', ');
}
function sortedSessions(sessions) {
    return [...sessions].sort((left, right) => left.startTime - right.startTime);
}
function dedupeStrings(values, limit = values.length) {
    const unique = values
...
function humanList(values, maxItems = values.length) {
...
function humanQuotedList(values, maxItems) {
...
function meaningfulQueryTokens(value) {
...
function isPriorWeekMention(normalized) {
    return normalized.includes('last week') || normalized.includes('past week');
}
function stripTrailingQueryAddOns(candidate) {
...
function stripKnownAppSuffixes(rawTitle) {
    return rawTitle.replace(/\s(?:-|\||—)\s(?:Visual Studio Code|VS Code|Cursor|Windsurf|Visual Studio|Rider|IntelliJ IDEA|PyCharm|WebStorm|Microsoft Outlook|Outlook|Google Chrome|Chrome|Microsoft Edge|Edge|Firefox|Excel|Word|PowerPoint|Windows Terminal|PowerShell|Command Prompt|Teams)$/i, '');
}
function looksLikeWorkArtifact(value) {
    return value.includes('/')
        || value.includes('\\')
        || /\.[a-z0-9]{1,6}\b/i.test(value);
}
function windowLeadLabel(label) {
...
function windowReferenceLabel(label) {
    if (label === 'past week')
        return 'over the past week';
    if (label.startsWith('the last '))
        return `over ${label}`;
...
function localhostLabel(domains) {
    const match = domains
        .map((domain) => domain.match(/localhost(?::\d{2,5})?/i)?.[0] ?? null)
        .find(Boolean);
    return match ?? 'localhost';
}
function strongestSessionObservation(session) {
    if (!session)
        return null;
    if (session.endMs - session.startMs < 20 * 60_000)
        return null;
...
function appDisplayNames(names) {
...
function terminalDisplayNames(names) {
    return dedupeStrings(names.map((raw) => {
        const lower = raw.toLowerCase();
        if (lower.includes('windows terminal'))
            return 'Windows Terminal';
        if (lower.includes('powershell') || lower === 'pwsh')
            return 'PowerShell';
        if (lower.includes('command prompt') || lower === 'cmd')
            return 'Command Prompt';
        if (lower.includes('wezterm'))
            return 'WezTerm';
        if (lower.includes('git bash'))
            return 'Git Bash';
        return raw;
    }));
}
function isTerminalLikeApp(session) {
...
function isCodingLikeApp(session) {
...
function isBrowserLikeSession(session) {
    if (session.category === 'browsing' || session.category === 'research')
        return true;
    const combined = `${session.appName} ${session.bundleId}`.toLowerCase();
...
function featureContext(titles, target) {
    ...
    return titles
        ...
        if (lower === targetLower || blocked.has(lower) || seen.has(lower))
            return false;
...
function visitEnd(visit) {
    return visit.visitTime + visit.durationSeconds * 1000;
}
function directEvidenceLabel(entity, session, visit) {
    if (session) {
        const title = session.windowTitle?.trim();
        if (title && textContainsEntity(title, entity))
            return title;
...
function mergeTimeRanges(ranges, maxGapMs = 0) {
    const sorted = [...ranges]
...
function buildDirectEvidence(entity, sessions, visits) {
    const evidence = [];
...
function clipVisitToWindows(visit, windows) {
    const overlaps = windows
        .map((window) => ({
...
function explicitWeekdayReference(normalized) {
    const weekdayNames = [
        { name: 'sunday', value: 0 },
        { name: 'monday', value: 1 },
...
function resolveExplicitWeekday(normalized, referenceDate) {
...
function hasTemporalCue(normalized) {
...
function resolveQuestionRange(question, anchorDate, preferAllTrackedTime = false) {
...
function cleanedEntityCandidate(value) {
...
function extractEntity(question) {
    ...
    for (const pattern of patterns) {
        ...
        if (meaningfulQueryTokens(cleaned).length > 0)
            return cleaned;
    }
    const acronym = question.match(/\b[A-Z][A-Z0-9&._-]{1,}\b/);
    return acronym?.[0] ?? null;
}
function isEntityQuestion(normalized, entity) {
    if (!entity)
        return false;
...
function extractAppFilter(question) {
    const normalized = question.toLowerCase();
...
function matchesAppFilter(session, appFilter) {
    if (!appFilter)
        return true;
    const haystack = normalizedText(`${session.appName} ${session.bundleId} ${session.windowTitle ?? ''}`);
    return haystack.includes(appFilter);
}
function matchesVisitAppFilter(visit, appFilter) {
    if (!appFilter)
        return true;
    const haystack = normalizedText(`${visit.browserBundleId ?? ''} ${visit.pageTitle ?? ''} ${visit.domain}`);
    return haystack.includes(appFilter);
}
function bucketForSession(session, visits) {
    if (isTerminalLikeApp(session))
        return 'terminal';
    if (isBrowserLikeSession(session))
        return 'browserTesting';
    if (isCodingLikeApp(session))
        return 'coding';
    if (session.category === 'aiTools' || isAIApp(session))
        return 'aiCollaboration';
    if (session.category === 'email' || session.category === 'productivity' || session.category === 'writing' || session.category === 'communication')
        return 'docs';
    if (visits.some((visit) => normalizedText(`${visit.domain} ${visit.url ?? ''}`).includes('localhost')))
        return 'browserTesting';
    return 'related';
}
function buildContributions(sessions, websiteVisits) {
    const grouped = new Map();
    for (const session of sessions) {
        const relatedVisits = websiteVisits.filter((visit) => (Math.max(session.startTime, visit.visitTime) < Math.min(sessionEnd(session), visitEnd(visit))));
        ...
        if (!current.appNames.includes(session.appName))
            current.appNames.push(session.appName);
        ...
        for (const visit of relatedVisits) {
            if (!current.domains.includes(visit.domain))
                current.domains.push(visit.domain);
...
function entityLead(entity, range, durationSeconds) {
    return `You put in ${formatDuration(durationSeconds)} on ${entity} ${windowLeadLabel(range.label)}.`;
}
function narrativeBullet(contribution, entity) {
    ...
    switch (contribution.bucket) {
        case 'coding': {
            ...
            if (strongestSessionPhrase)
                line += ` ${strongestSessionPhrase}`;
            return line;
        }
        case 'browserTesting': {
            let line;
...
function closingObservation(contributions, activeDays, entity) {
...
function buildEntityEvidence(entity, range, question, sessions, visits) {
    const appFilter = extractAppFilter(question);
    const directSessions = sessions.filter((session) => (matchesAppFilter(session, appFilter)
    ...
    const directWebsiteVisits = visits.filter((visit) => (matchesVisitAppFilter(visit, appFilter)
...
function buildEntitySuggestions(entity) {
...
function isTitleMatchQuestion(normalized) {
...
function isAppBreakdownQuestion(normalized) {
...
function buildEntityAppBreakdown(entity, range, activitySessions, websiteVisits) {
    if (activitySessions.length === 0 && websiteVisits.length === 0)
        return null;
    const grouped = new Map();
...
function buildEntityAnswer(evidence, question, sessionsInRange, resolvedContext) {
    const normalized = question.toLowerCase();
    ...
    const titleCoverageIsPartial = sessionsInRange.some((session) => ((session.category === 'email' || session.category === 'productivity' || session.category === 'writing')
        && !session.windowTitle));
    if (totalSeconds <= 0) {
        const answer = [
    ...
    if (isAppBreakdownQuestion(normalized)) {
        const answer = buildEntityAppBreakdown(evidence.entity, evidence.range, activitySessions, evidence.contextualWebsiteVisits);
        if (answer) {
            return {
                answer,
                resolvedContext,
    ...
    if (activitySessions.length === 0) {
        const examples = dedupeStrings(evidence.contextualWebsiteVisits.map((visit) => visit.pageTitle?.trim() || visit.domain), 4);
        const answer = [
...
function latestMeaningfulSession(sessions) {
    for (let index = sessions.length - 1; index >= 0; index--) {
        if (isMeaningfulSession(sessions[index]))
            return sessions[index];
    }
    return null;
}
function latestFocusedSession(sessions) {
    for (let index = sessions.length - 1; index >= 0; index--) {
        if (isFocusedSession(sessions[index]))
            return sessions[index];
    }
    return null;
}
function longestFocusedStretch(sessions) {
...
function buildEvidence(apps, sites, sessions) {
    return (0, workEvidence_1.deriveWorkEvidenceSummary)({
        appSummaries: apps,
        websiteSummaries: sites,
        sessions,
    });
}
function buildWorkThreadAnswer(apps, sites, sessions, resolvedPrefix) {
    if (apps.length === 0 && sites.length === 0 && sessions.length === 0)
        return null;
    ...
    if (totalSeconds === 0 && meaningfulSessions.length === 0)
        return null;
    ...
    for (const session of [...sortedSessions(meaningfulSessions)].reverse()) {
        ...
        if (!existing.includes(cleaned) && existing.length < 2)
...
function buildGeneralAppBreakdown(rangeLabel, apps, sites) {
    if (apps.length === 0)
        return null;
    const browserDomains = dedupeStrings(sites.map((site) => site.domain), 3);
    const browserTitles = dedupeStrings(sites
...
function buildGeneralTitleAnswer(rangeLabel, sessions, sites) {
...
function buildTimeAllocationAnswer(apps, sites) {
    if (apps.length === 0 && sites.length === 0)
        return null;
    const evidence = buildEvidence(apps, sites, []);
    const strongestFocus = evidence.signals.filter((signal) => types_1.FOCUSED_CATEGORIES.includes(signal.category)).slice(0, 3);
...
function buildDistractionAnswer(apps, sites) {
    if (apps.length === 0 && sites.length === 0)
        return null;
...
function buildFocusScoreAnswer(apps, sessions, sites) {
    if (apps.length === 0 && sessions.length === 0 && sites.length === 0)
        return null;
    const evidence = buildEvidence(apps, sites, []);
    const totalSeconds = Math.max(evidence.totalSeconds, apps.reduce((sum, app) => sum + app.totalSeconds, 0));
    const focusSeconds = evidence.focusedSeconds;
    const switchesPerHour = totalSeconds > 0 ? Math.max(0, sessions.length - 1) / Math.max(totalSeconds / 3600, 0.25) : 0;
    const score = (0, focusScore_1.computeFocusScore)({
...
function buildTimelineSummary(apps, sites, sessions) {
    if (sessions.length === 0 && apps.length === 0 && sites.length === 0)
        return null;
    const evidence = buildEvidence(apps, sites, []);
...
function resolveTargetDate(question, defaultDate, previousContext) {
    const normalized = question.toLowerCase();
...
function shiftWindowToDate(window, date) {
...
function resolveTimeWindow(question, resolvedDate, previousContext) {
    const normalized = question.toLowerCase();
...
function resolveTemporalContext(question, defaultDate, previousContext) {
...
function durationMatchAnswer(normalized, apps, sites) {
...
function exactMomentAnswer(window, sessions, sites) {
    if (sessions.length === 0)
        return 'No tracked activity in that time window.';
...
function timeRangeAnswer(window, sessions, sites) {
...
function isEntityIdentityQuestion(normalized) {
...
function buildEntityIdentityAnswer(evidence, resolvedContext) {
...
function extractEntitiesFromActivity(sessions, visits) {
    const entitySeconds = new Map();
    function addEntity(entity, seconds) {
        if (ENTITY_STOP_WORDS.has(entity.toUpperCase()))
            return;
        if (entity.length < 2 || entity.length > 30)
            return;
        if (APP_DISPLAY_NOISE.has(entity))
            return;
    ...
    for (const session of sessions) {
        ...
        for (const match of title.matchAll(/\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,3})\b/g)) {
            if (!APP_DISPLAY_NOISE.has(match[1]))
                addEntity(match[1], session.durationSeconds);
        }
    }
    for (const visit of visits) {
...
function isClientListQuestion(normalized) {
...
function buildClientListAnswer(rangeLabel, sessions, visits, resolvedContext) {
    const meaningfulSessions = sessions.filter(isMeaningfulSession);
    ...
    const lines = ranked.map(([entity, seconds], index) => `${index + 1}. ${entity}: ${formatDuration(seconds)}`);
...
function isComparisonQuestion(normalized) {
    return /\bvs\.?\s|\bversus\b|\bcompare\b|\bcompared\s+to\b/.test(normalized);
}
function extractTwoEntities(question) {
    const vsMatch = question.match(/(.+?)\s+(?:vs\.?|versus|compared?\s+to)\s+(.+?)(?:\s+(?:today|yesterday|this week|that week|last week|past week|last \d+ days?))*\s*\??$/i);
    if (!vsMatch)
        return null;
    ...
    if (!a || !b || meaningfulQueryTokens(a).length === 0 || meaningfulQueryTokens(b).length === 0)
        return null;
    return [a, b];
}
function buildComparisonAnswer(entityA, entityB, range, sessions, visits, resolvedContext) {
...
function isDaySummaryQuestion(normalized) {
...
function buildDaySummaryAnswer(apps, sites, sessions, visits, rangeLabel) {
    if (apps.length === 0 && sessions.length === 0)
        return null;
    const totalSeconds = apps.reduce((sum, app) => sum + app.totalSeconds, 0);
    ...
    if (topAppsLine)
        parts.push(`Top apps: ${topAppsLine}.`);
    if (entityLine)
        parts.push(`Clients/projects in titles: ${entityLine}.`);
    if (topSite)
        parts.push(`Most visited site: ${topSite.domain} (${formatDuration(topSite.totalSeconds)}).`);
    if (latestSession) {
...
async function routeInsightsQuestion(question, defaultDate, previousContext, db) {
    ...
    if (isClientListQuestion(normalized)) {
        const sessions = (0, queries_1.getSessionsForRange)(db, dayRange.startMs, dayRange.endMs + 1);
        const visits = (0, queries_1.getWebsiteVisitsForRange)(db, dayRange.startMs, dayRange.endMs + 1);
    ...
    if (normalized.includes('focus score') || /\bwas\s+i\s+focused\b/.test(normalized)) {
        ...
        const appsMetric = (0, queries_1.getAppSummariesForRange)(db, fromMsMetric, toMsMetric);
        const sessionsMetric = (0, queries_1.getSessionsForRange)(db, fromMsMetric, toMsMetric);
        const sitesMetric = (0, queries_1.getWebsiteSummariesForRange)(db, fromMsMetric, toMsMetric);
    ...
    if (isWeeklyQuestion(normalized)) {
        ...
        const apps = (0, queries_1.getAppSummariesForRange)(db, weeklyRange.startMs, weeklyRange.endMs + 1);
        const sites = (0, queries_1.getWebsiteSummariesForRange)(db, weeklyRange.startMs, weeklyRange.endMs + 1);
        const sessions = (0, queries_1.getSessionsForRange)(db, weeklyRange.startMs, weeklyRange.endMs + 1);
        const visits = (0, queries_1.getWebsiteVisitsForRange)(db, weeklyRange.startMs, weeklyRange.endMs + 1);
        if (isClientListQuestion(normalized)) {
    ...
    if (resolvedContext.timeWindow) {
        const sessions = (0, queries_1.getSessionsForRange)(db, resolvedContext.timeWindow.start.getTime(), resolvedContext.timeWindow.end.getTime());
        const sites = (0, queries_1.getWebsiteSummariesForRange)(db, resolvedContext.timeWindow.start.getTime(), resolvedContext.timeWindow.end.getTime());
        const answer = timeRangeAnswer(resolvedContext.timeWindow, sessions, sites);
        return answer ? { answer, resolvedContext } : null;
    }
    // ── Day-scoped routes ───────────────────────────────────────────────────────
    const [fromMs, toMs] = dayBounds(resolvedContext.date);
    const apps = (0, queries_1.getAppSummariesForRange)(db, fromMs, toMs);
    const sites = (0, queries_1.getWebsiteSummariesForRange)(db, fromMs, toMs);
    const sessions = (0, queries_1.getSessionsForRange)(db, fromMs, toMs);
    const visits = (0, queries_1.getWebsiteVisitsForRange)(db, fromMs, toMs);
    ...
    if (normalized.includes('most used app')
    ...
    if (normalized.includes('most used site')
